          ...
          <div className="mt-12 pt-8 border-t border-gray-200">
            <div className="flex flex-wrap items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-2 text-gray-500">
                ...
                <span className="text-sm">{new Date(article.createdAt).toDateString()}</span>
              </div>
            </div>
          </div>
...
{payload ? (<AddCommentForm articleId={parseInt(id)} />):<p className="text-muted">Please log in to add a comment.</p>}
          <div>
      {article.comments.map((comment) => (
        <CommentItem key={comment.id} comment={comment}   userId={payload?.userId}/>
      ))}
    </div>
  </div>
  );
